 ...
 */
abstract class TDProject_Core_Controller_DispatchAction 
	extends TechDivision_Controller_Action_Abstract {
	
	/**
	 * The default method suffix.
	 * @var string
	 */
    ...
    public function perform() {
		...
		// create the method name to invoke
		$methodName = $this->_getMethodName($method);
		...
		if (!method_exists($this, $methodName)) {
			...
			throw new TechDivision_Controller_Exceptions_MethodNotFoundException(
				'Specified method ' . $methodName . ' not implemented by class ' . $this->_getClass()
			);
		}
    ...
    /**
     * Appends the default Suffix to the passed method name and returns
     * the new method name.
     * 
     * @param string $method The method name to append the suffix to
     * @return string The method name appended with the suffix
     */
