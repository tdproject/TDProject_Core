...
(function(o, K) {
	...
	function Wa(a, b, f, c) {
			...
			if (U) {
				...
				U.find("td").each(
						function() {
							...
							if (N > 1)
								B.getMonth() == s ? W
										.removeClass("fc-other-month") : W
										.addClass("fc-other-month");
							+B == +I ? W.removeClass("fc-not-today").addClass(
									"fc-today").addClass(
									ca + "-state-highlight") : W.addClass(
									"fc-not-today").removeClass("fc-today")
									.removeClass(ca + "-state-highlight");
				...
				m = "<thead><tr>";
				for (e = 0; e < L; e++) {
					m += "<th class='fc-" + za[B.getDay()] + " " + ca
							+ "-state-default" + (e == R ? " fc-leftmost" : "")
							+ "'>" + oa(B, j, b) + "</th>";
					C(B, 1);
					O && da(B)
				}
				...
				for (e = 0; e < N; e++) {
					...
					for (E = 0; E < L; E++) {
						m += "<td class='fc-"
								...
								+ (e * L + E)
								+ (E == R ? " fc-leftmost" : "")
								+ (N > 1 && B.getMonth() != s ? " fc-other-month"
										: "")
								+ (+B == +I ? " fc-today " + ca
										+ "-state-highlight" : " fc-not-today")
								+ "'>"
								+ (r ? "<div class='fc-day-number'>"
										+ B.getDate() + "</div>" : "")
								+ "<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>";
						C(B, 1);
						O && da(B)
				...
				fa(U.find("td"));
				P = o("<div style='position:absolute;z-index:8;top:0;left:0'/>")
						.appendTo(a)
			}
		}
		function n(m) {
			...
			var e = Fa - x.height(), j;
			if (b.weekMode == "variable")
				j = e = Math.floor(e / (N == 1 ? 2 : 6));
			else {
				j = Math.floor(e / N);
				e = e - j * (N - 1)
			}
		...
		var ca, ua, O, ra, ka, R, va, Fa, N, L, Aa, x, U, wa = [], P, la = new kb(
				function(m) {
					return U.find("td:eq(" + (m - Math.max(ua, O) + L) % L
							+ ") div div")
	...
	function jb(a, b, f, c, g, n, k, y, w, t, H) {
		...
		for (G = 0; G < fa; G++) {
			...
			ga += "<div class='"
					...
					+ (Q.url ? " href='" + Ka(Q.url) + "'" : "")
					+ ">"
					+ (!Q.allDay && D.isStart ? "<span class='fc-event-time'>"
							+ Ka(Ha(Q.start, Q.end, f.option("timeFormat"), M))
							+ "</span>" : "")
					+ "<span class='fc-event-title'>"
					+ Ka(Q.title)
					+ "</span></a>"
					+ ((Q.editable || Q.editable === K && M.editable)
							&& !M.disableResizing && o.fn.resizable ? "<div class='ui-resizable-handle ui-resizable-"
							+ (sa ? "w" : "e") + "'></div>"
							: "") + "</div>";
			D.left = ma;
			D.outerWidth = pa - ma
		...
		for (G = 0; G < fa; G++) {
			D = a[G];
			if (c = D.element)
				c[0].style.width = D.outerWidth - D.hsides + "px"
		}
		for (G = 0; G < fa; G++) {
			D = a[G];
			if (c = D.element) {
				t = ca[w = D.key];
				D.outerHeight = c[0].offsetHeight
						+ (t === K ? (ca[w] = tb(c[0])) : t)
			}
		}
		for (X = G = 0; X < b; X++) {
			...
			ua[X] = n(X).find("td:first div.fc-day-content > div").height(
					ca + t)
		}
		for (X = 0; X < b; X++)
			O[X] = ua[X][0].offsetTop;
		for (G = 0; G < fa; G++) {
			D = a[G];
			if (c = D.element) {
	...
	function ub(a, b, f, c) {
		function g(d, i) {
			...
			if (A) {
				G();
				A.find("tr:first th").slice(1, -1).each(
						function() {
				...
				ia.find("td").each(
						function() {
							this.className = this.className.replace(
									/^fc-\w+(?= )/, "fc-" + za[h.getDay()]);
							+h == +p ? o(this).removeClass("fc-not-today")
									.addClass("fc-today").addClass(
											na + "-state-highlight") : o(this)
									.addClass("fc-not-today").removeClass(
				...
				var l, u, z = b.slotMinutes % 15 == 0, F = "<div class='fc-agenda-head' style='position:relative;z-index:4'><table style='width:100%'><tr class='fc-first"
						+ (b.allDaySlot ? "" : " fc-last")
						+ "'><th class='fc-leftmost "
						+ na
						+ "-state-default'>&nbsp;</th>";
				for (l = 0; l < m; l++) {
					F += "<th class='fc-" + za[h.getDay()] + " " + na
							+ "-state-default'>" + oa(h, i, b) + "</th>";
					C(h, Y);
					La && da(h, Y)
				}
				F += "<th class='" + na + "-state-default'>&nbsp;</th></tr>";
				if (b.allDaySlot)
					F += "<tr class='fc-all-day'><th class='fc-axis fc-leftmost "
							+ na
							+ "-state-default'>"
							+ b.allDayText
							+ "</th><td colspan='"
				...
				A = o(F).appendTo(a);
				w(A.find("td"));
				W = o("<div style='position:absolute;z-index:8;top:0;left:0'/>")
						.appendTo(A);
				h = xb();
				var T = ba(q(h), Ua);
				ba(h, Da);
				...
				aa = o(
						...
						.append(
								Z = o(
										...
										.append($ = o(F))).appendTo(a);
				t(aa.find("td"));
				Ba = o(
						"<div style='position:absolute;z-index:8;top:0;left:0'/>")
						.appendTo(Z);
				h = q(d);
				F = "<div class='fc-agenda-bg' style='position:absolute;z-index:1'><table style='width:100px;height:100px'><tr class='fc-first'>";
				for (l = 0; l < m; l++) {
					F += "<td class='fc-"
							+ za[h.getDay()]
							+ " "
							...
							+ "-state-default "
							+ (!l ? "fc-leftmost " : "")
							+ (+h == +p ? na + "-state-highlight fc-today"
									: "fc-not-today")
		...
		function y(d) {
			...
			$.width(h);
			j = 0;
			Ja(A.find("tr:lt(2) th:first").add(aa.find("tr:first th"))
					.width("").each(function() {
		...
		function H(d) {
			if (!v.option("selectable")) {
				var i = Math.min(m - 1, Math.floor((d.pageX - ia.offset().left)
						/ r));
				...
				if (h) {
					...
					v.trigger("dayClick", this, i, true, d)
			}
		}
		function M(d, i) {
			v.reportEvents(J = d);
			...
			for (h = 0; h < p; h++)
				d[h].allDay ? l.push(d[h]) : u.push(d[h]);
			Q(fa(l), i);
			ha(D(u), i)
		}
		function sa(d) {
			...
			for (h = 0; h < p; h++) {
				...
				if (z = l.element) {
					...
					if (l.titleTop !== K && Ma - l.titleTop < 10) {
						z.find("span.fc-event-time").text(
								oa(u.start, v.option("timeFormat")) + " - "
										+ u.title);
		...
		function ma(d, i, h) {
			return "<div class='"
					...
					+ Ka(Ha(d.start, d.end, v.option("timeFormat")))
					+ "</span><span class='fc-event-title'>"
					+ Ka(d.title)
					+ "</span></a>"
					+ ((d.editable || d.editable === K && b.editable)
							&& !b.disableResizing && o.fn.resizable ? "<div class='ui-resizable-handle ui-resizable-s'>=</div>"
							: "") + "</div>"
		}
		function pa(d, i, h) {
						...
						.draggable( {
														...
														if (S.row) {
															if (h && l) {
																Sa(
																		...
																		s
																				* Math
																						.round((d.end ? (d.end - d.start)
																								/ Hb
																								: b.defaultEventMinutes)
																								/ b.slotMinutes));
																i
																		.draggable(
																				"option",
							...
							stop : function(F, T) {
								...
								if (S && (!l || u)) {
									...
									l
											|| (S = Math
													.round((i.offset().top - Z
															.offset().top)
															/ s)
													* b.slotMinutes
													+ Da
													- (d.start.getHours() * 60 + d.start
															.getMinutes()));
		...
		function ca(d, i, h) {
			if (!b.disableDragging && i.draggable) {
				...
				i.draggable( {
					...
					drag : function(V, ea) {
						z = Math.round((ea.position.top - p.top) / s)
								* b.slotMinutes;
						if (z != F) {
							l || T(z);
							F = z
					...
					stop : function(V, ea) {
						...
						if (ja && (u || z || l))
							v.eventDrop(this, d, u, l ? 0 : z, l, V, ea);
						else {
							S();
							i.css(p);
							T(0);
				...
				i
						.resizable( {
							...
							start : function(u, z) {
								p = l = 0;
								v.hideEvents(d, i);
								o.browser.msie && o.browser.version == "6.0"
							...
							resize : function(u, z) {
								p = Math
										.round((Math.max(s, i.height()) - z.originalSize.height)
												/ s);
								if (p != l) {
									h.text(Ha(d.start, !p && !d.end ? null
											: ba(v.eventEnd(d), b.slotMinutes
													* p), v
											.option("timeFormat")));
									l = p
								}
		...
		function O(d, i) {
			...
			if (p === K)
				p = $a[h] = aa.find("tr:eq(" + h + ") td div")[0].offsetTop;
			...
			if (v.option("selectable")) {
				...
				Ea.start(function(p, l) {
					...
					if (p && p.col == l.col && !la(p)) {
						l = P(l);
						p = P(p);
						h = [ l, ba(q(l), b.slotMinutes), p,
								ba(q(p), b.slotMinutes) ].sort(zb);
						va(h[0], h[3])
					} else
						h = null
				}, d);
		...
		var A, aa, Z, $, ia, m, e = 0, j, r, s, I, E, B, J = [], W, Ba, na, vb, La, Ta, Y, ta, Da, Ua, Qa = new kb(
				function(d) {
		...
		var Ra = new nb(function(d, i) {
			...
			for ( var z = Z.offset().top, F = aa.offset().top, T = F
					+ aa.outerHeight(), S = 0; S < e; S++)
				d.push( [ h(z + s * S), h(z + s * (S + 1)) ])
		...
		for (b = a.length - 1; b > 0; b--) {
			...
			for (f = 0; f < g.length; f++) {
				...
				for (c = 0; c < a[b - 1].length; c++) {
					k = a[b - 1][c];
					if (Ab(n, k))
						k.forward = Math.max(k.forward || 0,
								(n.forward || 0) + 1)
				}
			}
		}
	...
	function qb(a, b) {
		a.option("selectable") && a.option("unselectAuto")
				&& o(document).mousedown(function(f) {
					var c = a.option("unselectCancel");
					if (c)
						if (o(f.target).parents(c).length)
							return;
					b(f)
				})
	}
	...
	function eb(a, b, f) {
		if (+a) {
			...
			c.setMonth(b);
			a.setMonth(b);
			for (f || Ga(a); a.getMonth() != c.getMonth();)
				a.setDate(a.getDate() + (a < c ? 1 : -1))
		}
		return a
	}
	function C(a, b, f) {
	...
	function da(a, b, f) {
		for (b = b || 1; !a.getDay() || f && a.getDay() == 1 || !f
				&& a.getDay() == 6;)
			C(a, b);
		return a
	}
	...
	o.fn.fullCalendar = function(a) {
		...
		c.unshift( []);
		a = o.extend(true, {}, Va, a.isRTL || a.isRTL === K && Va.isRTL ? Mb
				: {}, a);
		var g = a.theme ? "ui" : "fc";
		this
				.each(function() {
								...
								x = U[e] = ya.views[e]
										(
												r = wa = o(
														...
														.appendTo(R), a, e);
							if ($) {
								$.find("div.fc-button-" + Aa).removeClass(
										g + "-state-active");
								$.find("div.fc-button-" + e).addClass(
										g + "-state-active")
							}
							Aa = e;
							k();
					...
					function k(e) {
						if (y()) {
							...
							if (!x.start || e || L < x.start || L >= x.end) {
								x.render(L, e || 0);
								X(true);
								!la || !a.lazyFetching || x.visStart < la
										|| x.visEnd > A ? Q() : x
										.renderEvents(P)
							} else if (x.sizeDirty || x.eventsDirty
									|| !a.lazyFetching) {
								x.clearEvents();
								x.sizeDirty && X();
							...
							if ($) {
								...
								e = new Date;
								e >= x.start && e < x.end ? $.find(
										"div.fc-button-today").addClass(
										g + "-state-disabled") : $.find(
										"div.fc-button-today").removeClass(
										g + "-state-disabled")
							}
							N--;
					...
					function D(e, j) {
						...
						if (typeof e == "string") {
							...
							if (a.cacheParam)
								B[a.cacheParam] = (new Date).getTime();
							ha();
							o.ajax( {
								url : e,
								dataType : "json",
					...
					function ga() {
						va = a.contentHeight ? a.contentHeight
								: a.height ? a.height - ($ ? $.height() : 0)
										- bb(R[0]) : Math.round(R.width()
										/ Math.max(a.aspectRatio, 0.5))
					}
					function X(e) {
						N++;
						x.setHeight(va, e);
					...
					var O = this, ra = o(O).addClass("fc"), ka, R = o(
							"<div class='fc-content "
									+ g
									+ "-widget-content' style='position:relative'/>")
							.prependTo(O), va, Fa = 0, N = 0, L = new Date, Aa, x, U = {}, wa;
					...
					var P = [], la, A, aa = 0, Z = {
						...
						updateEvent : function(e) {
							var j, r = P.length, s, I = e.start - e._start, E = e.end ? e.end
									- (e._end || x.defaultEventEnd(e))
									: 0;
							for (j = 0; j < r; j++) {
								s = P[j];
								if (s._id == e._id && s != e) {
									s.start = new Date(+s.start + I);
									s.end = e.end ? s.end ? new Date(+s.end + E)
											: new Date(+x.defaultEventEnd(s)
													+ E)
											: null;
									s.title = e.title;
									s.url = e.url;
					...
					var $, ia = a.header;
					if (ia)
						$ = o("<table class='fc-header'/>").append(
								o("<tr/>").append(
										o("<td class='fc-header-left'/>")
												.append(pa(ia.left))).append(
										o("<td class='fc-header-center'/>")
												.append(pa(ia.center))).append(
										o("<td class='fc-header-right'/>")
												.append(pa(ia.right))))
								.prependTo(ra);
					o(window).resize(ca);
					if (a.droppable) {
	...
	Oa.month = function(a, b, f) {
		return new Wa(a, b, {
			render : function(c, g) {
				...
				this.renderGrid(n, b.weekends ? 7 : 5, this
	...
	Oa.basicWeek = function(a, b, f) {
		return new Wa(a, b, {
			render : function(c, g) {
				g && C(c, g * 7);
				c = this.visStart = q(this.start = C(q(c), -((c.getDay()
						- b.firstDay + 7) % 7)));
	...
	var lb = {
		...
		trigger : function(a, b) {
			if (this.options[a])
				return this.options[a].apply(b || this, Array.prototype.slice
		...
		renderOverlay : function(a, b) {
			...
			f
					|| (f = o("<div class='fc-cell-overlay' style='position:absolute;z-index:3'/>"));
			f[0].parentNode != b[0] && f.appendTo(b);
			this.usedOverlays.push(f.css(a).show());
			return f
		},
		clearOverlays : function() {
			for ( var a; a = this.usedOverlays.shift();)
				this.unusedOverlays.push(a.hide().unbind())
		},
		resizableDayEvent : function(a, b, f) {
			var c = this;
			if (!c.options.disableResizing && b.resizable)
				b.resizable( {
					...
					stop : function(g, n) {
						...
						var k = Math.round((b.width() - n.originalSize.width)
								/ f);
						if (k)
							c.eventResize(this, a, k, 0, g, n);
						else {
							b.css("z-index", 8);
							c.showEvents(a, b)
						}
			...
			b.click(
					function(c) {
						if (!b.hasClass("ui-draggable-dragging")
							...
							return f.trigger("eventClick", this, a, c)
					}).hover(function(c) {
	...
	var ib = ya.parseDate = function(a) {
		...
		if (typeof a == "string") {
			if (a.match(/^\d+$/))
				return new Date(parseInt(a) * 1E3);
